...
![lightprobe_compare]({{ site.img_url }}/lightprobe_compare.jpg)
...
对应即`Engine\Content\MapTemplates\Sky\DaylightAmbientCubemap.uasset`，这种蛋疼的uasset在之前的日志[拆解Unreal资源]({% post_url 2014-11-7-unreal-resources %})中已简单分析过。这次的比较简单，随便拷贝到一个Unreal项目文件夹下，然后引擎会自动读取，右键Export就可以得到一个HDR格式的资源文件。
...
![lightprobe_hdrshop]({{ site.img_url }}/lightprobe_hdrshop.png)
...
![lightprobe_hdrshop]({{ site.img_url }}/lightprobe_ue4.png)

![lightprobe_hdrshop_cubecross]({{ site.img_url }}/lightprobe_hdrshop_cubecross.png)
...
![lightprobe_hdrshop_cubecross]({{ site.img_url }}/lightprobe_Earthmap720x360_grid.jpg)
- Cubic Environment: 使用六张图组成的环境贴图，参考刚才的输出图，不过和我们平常用的一横条六个图不一样，这里是组成了一个Vertical Cross(竖十字架)；
- Mirrored Ball: 反光球
![lightprobe_hdrshop_mirrorball]({{ site.img_url }}/lightprobe_hdrshop_mirrorball.png)
- Light Probe: 光探针
![lightprobe_hdrshop_lightprobe]({{ site.img_url}}/lightprobe_hdrshop_lightprobe.png)
...
![lightprobe_cubemangen1]({{ site.img_url}}/lightprobe_cubemangen1.png)
...
对比图(MipMap开的比较高)：
![lightprobe_cubemangen2]({{ site.img_url}}/lightprobe_cubemangen2.png)
...
![lightprobe_unity3d]({{ site.img_url}}/lightprobe_unity3d.png)